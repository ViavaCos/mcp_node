...
// ---------- 脚本化降级演示（无需 LLM 密钥）----------
function bar(items, labelKey, valueKey, unit = '') {
  const max = Math.max(...items.map((x) => x[valueKey]), 1);
  return items
    .map((x) => {
      const len = Math.round((x[valueKey] / max) * 28);
      return `  ${String(x[labelKey]).padEnd(10)} ${'█'.repeat(len)} ${unit}${x[valueKey].toLocaleString('zh-CN')}`;
    })
    .join('\n');
}
...
async function runDemo() {
  ...
  const cat = await callTool('get_sales_ranking', { type: 'category', period: '2025', limit: 8 });
  console.log(bar(cat.items, 'name', 'totalAmount', '¥'));

  console.log('\n【问题 2】2025 年度整体表现如何？(get_annual_summary)');
  const sum = await callTool('get_annual_summary', { year: '2025' });
  ...
  console.log('  月度趋势：' + sum.monthlyTrend.map((m) => m.month + '月' + Math.round(m.amount / 1000) + 'k').join(' '));

  console.log('\n【问题 3】想上架“手机”类新品，现有商品有哪些？(query_products)');
  const prod = await callTool('query_products', { category: 'c1', keyword: '手机', limit: 5 });
  console.log(`  命中 ${prod.total} 个，样例：`);
  prod.items.slice(0, 5).forEach((p) => console.log(`   - ${p.name}  ¥${p.price}  (${p.brand})`));

  console.log('\n【问题 4】华南地区 2025 的头部客户是谁？(get_active_users)');
  const users = await callTool('get_active_users', { year: '2025', region: '华南', limit: 5 });
  console.log(bar(users.items, 'name', 'totalAmount', '¥'));

  console.log('\n【问题 5】2024 vs 2025，华东地区订单量变化？(query_orders 两次取 total)');
  const o24 = await callTool('query_orders', { region: '华东', year: '2024', pageSize: 1 });
  const o25 = await callTool('query_orders', { region: '华东', year: '2025', pageSize: 1 });
